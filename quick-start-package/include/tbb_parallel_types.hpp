...
/* Struct for parallelizing create table t */
#ifdef g
struct TableConstructionT {

public:
    uint64_t my_sum;

    /* Initial constructor */
    TableConstructionT(uint64_t * dataPtr, unsigned * row_ids, unsigned rnum, int idx)
    :col{dataPtr}, rids{row_ids}, rels_num{rnum}, tbi{idx}, my_sum(0)
    {}

    /* Slpitting constructor */
    TableConstructionT(TableConstructionT & x, split)
    :col{x.col}, rids{x.rids}, rels_num{x.rels_num}, tbi{x.tbi}, my_sum(0) {}

    /* How to join the thiefs */
    void join(const TableConstructionT & y) {my_sum += y.my_sum;}
