...
relation64_t * CreateRowRelationT_t64(uint64_t * column, unsigned size, JobScheduler &js1, JobScheduler &js2) {
    ...
    for (size_t i = 0; i < range; i++) {
        ...
        a[i].tups = new_relation->tuples;
        if (i % 2 == 0 )
            js1.Schedule(new JobCreateNonInterRel_t64(a[i]));
        else
            js2.Schedule(new JobCreateNonInterRel_t64(a[i]));
