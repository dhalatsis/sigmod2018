...
public:
    ...
    private:
        ...
        // thread function for numa region 0
        static void* CallThrFn_CPU3(void *pthis) {

            JobExecutor *executor = static_cast<JobExecutor*>(pthis);
            JobMaster *JobMaster = nullptr;
            Joiner joiner;
            joiner.job_scheduler.Init(THREAD_NUM_1CPU, NUMA_REG1 + 2);
        ...
        // thread function for numa region 0
        static void* CallThrFn_CPU4(void *pthis) {

            JobExecutor *executor = static_cast<JobExecutor*>(pthis);
            JobMaster *JobMaster = nullptr;
            Joiner joiner;
            joiner.job_scheduler.Init(THREAD_NUM_2CPU, NUMA_REG2 + 2);
            ...
            //std::cerr << "Query About to execute by master 1" << '\n';
